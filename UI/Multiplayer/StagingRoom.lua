...
function RealizeGameSetup()
	...
	if (GameConfiguration.GetGameState() == GameStateTypes.GAMESTATE_PREGAME
		and (GameConfiguration.GetValue("RULESET") == "RULESET_EXPANSION_1" or GameConfiguration.GetValue("RULESET") == "RULESET_EXPANSION_2")) then
		local slotID		= 62
        local playerConfig  = PlayerConfigurations[slotID]
        if playerConfig then
